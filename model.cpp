...
Model::Model(const char *filename) : verts_(), faces_() {
    ...
    while (!in.eof()) {
        ...
        if (!line.compare(0, 2, "v ")) {
            ...
            Vec3f v;
            for (int i=0;i<3;i++) iss >> v.raw[i];
            textureVerts_.push_back(v);
        }
    }
    std::cerr << "# v# " << verts_.size() << " f# "  << faces_.size() << " tv# " << textureVerts_.size() << std::endl;
}
